...
const COPY = {
  ja: {
    mainPanel: {
      heroTitle: "UUIDv4の衝突を検証する\n毎秒ひたすら観測する",
      totalAttemptsLabel: "試行回数",
      totalAttemptsDescription: "これまでに生成してデータベースに保存した総数",
      totalUniqueUuidsLabel: "一意UUID数",
      totalUniqueUuidsDescription: "まだ一度しか観測されていないUUIDの累積",
      totalCollisionsLabel: "衝突回数",
      totalCollisionsDescription: "既出UUIDと一致したイベントの累積",
    },
    recent: {
      eyebrow: "Recent Attempts",
      title: "直近の生成イベント",
      description: "手動追加を押すと、１件だけUUIDv4を追加できます。",
    ...
    search: {
      eyebrow: "UUID Search",
      title: "生成済みUUIDを検索する",
      description: "部分一致でも探せます。空欄のままなら最近観測したUUIDを表示します。",
    ...
    unobservedLabel: "未観測",
    latestAttemptNone: "まだ試行履歴はありません。ワーカーを起動すると、１秒ごとに記録が積み上がります。",
    latestAttemptCollision: (uuid: string): string => `衝突発生。\n${uuid} が再び観測されました。`,
    latestAttemptUnique: (uuid: string): string => `最新UUIDは ${uuid} です。\nいまのところ衝突は確認されていません。`,
    ...
    manualTriggerError: "手動追加に失敗しました。コンテナとDBの状態を確認してください。",
  },
  en: {
    mainPanel: {
